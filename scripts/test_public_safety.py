...
def _run() -> int:
    ...
    with contextlib.redirect_stdout(boot_log):
        ...
        rt = build_lab(
            # max_total=7: under ADR-014 the digest drafts once, so the
            # session makes few LLM calls — 7 leaves plan one proposal and
            # still exhausts mid-run (each keyed live finding adds a call;
            # paused-boot, model-parameter-compatibility, and the ADR-036
            # source-selection self-repair finding did, and the seeded
            # branch's gap-blocked task now spends one interpret call too —
            # blocked is an outcome since the branch#64 fix).
            llm_provider=LabProviderWrapper(LabMockProvider(), max_total=7,
