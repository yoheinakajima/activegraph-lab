...
- When the brief describes a DEFECT, you MUST also add or extend a regression test that fails WITHOUT your fix and passes WITH it. The lab runs that test DIRECTLY with pytest (the proof command alone may not run it), and a test that does not actually execute-and-pass fails the proof — so a test-only change that does not fix the underlying source is REJECTED. Make the test genuinely exercise the bug: it must fail against the unfixed source and pass once your fix is applied. Emit that test file's full contents too.
