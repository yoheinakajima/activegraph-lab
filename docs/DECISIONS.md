...
- Note (2026-06-11, connection resilience): the upstream PostgresEventStore holds a single boot-lifetime connection; serverless Postgres (Neon) suspends idle compute and terminates every connection with it. Production signature, twice: first post-idle write fails `AdminShutdown` ("terminating connection due to administrator command"), every later write fails `OperationalError` ("the connection is closed") until restart — ADR-023 surfaced both structurally, nothing committed. `lab_pack/storage.harden_store` (this module is already the one backend-aware place) wraps store operations: a connection-class error (`OperationalError`/`InterfaceError`, SSL EOF, connection reset) re-establishes the connection and retries the operation exactly once; a second failure surfaces structured (ADR-023); non-connection errors (`UniqueViolation`) are never retried — which also makes a retried append double-commit-safe via `UNIQUE(id, run_id)`. Each reconnect records `store_reconnected` on the diagnostics ring buffer — never the event log, which is precisely what a dead connection breaks. Long-idle appends get a cheap `SELECT 1` probe first. Upstream candidate seeded as a LIVE_FINDINGS observation: reconnect-with-bounded-retry belongs in the store itself.
