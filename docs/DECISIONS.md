...
- Note (2026-06-11, connection resilience): PostgresEventStore holds a single boot-lifetime connection; serverless Postgres (Neon) suspends idle computes and kills it. Observed twice in production with the identical signature — first write after an idle suspend fails AdminShutdown, every subsequent write fails OperationalError ("the connection is closed") until restart; ADR-023 surfaced both correctly, nothing committed. `storage.harden_store` (armed at server boot) wraps the store's operations: on a connection-class error (psycopg OperationalError — which covers AdminShutdown and SSL EOF/reset — or InterfaceError) it re-establishes the connection and retries the operation exactly once; a second failure, or a failed reconnect, surfaces structured per ADR-023. Non-connection errors are never retried — a retried append whose first attempt actually committed must surface as UniqueViolation, not duplicate. Each reconnect records a `store_reconnected` entry on the diagnostics ring buffer, never the event log (the log may be the casualty — ADR-023). Touching the store's connection internals stays legal here and only here; the upstream candidate (reconnect-with-bounded-retry belongs in the store) is queued in LIVE_FINDINGS.
