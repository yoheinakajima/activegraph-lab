...
- `seam_writer` — llm_behavior (Phase 4 rails). On seam_proposal_request observations — assembled by answer's "propose …" steering intent with the seam's current version verbatim plus cited evidence (the operator's message, rejected decisions) — authors the next seam version through propose_seam_fn: artifact + PENDING self_modify decision whose evidence_refs record what informed it. Nothing auto-applies; the reserved prompt.draft_writer voice episode stays reserved.
