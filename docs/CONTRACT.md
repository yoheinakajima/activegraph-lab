...
- Self-dispatched repair (ADR-036/037): the planner may turn the lab's OWN observed defects (a `routing_miss`; a finding the build tagged `code_defect`) into a GATED code-fix branch routed `codebase.code_task`, carrying the defect's evidence — lab-repairs-lab, not operator-hands-lab-each-bug. On activation the code worker AUTHORS the fix (ADR-037, amended by ADR-038 then ADR-042): a propose_pr task with a brief but no candidate diff drives an LLM authoring step (on `setting.model.code_worker`) that reads the cloned files and emits, per file, search/replace EDITS (a verbatim block to find + its replacement) to MODIFY an existing file, or full CONTENT to CREATE a new file (e.g. a regression test) — NEVER a unified diff. The lab MATERIALIZES the new file contents (apply each edit to the cloned original) and constructs the patch DETERMINISTICALLY (difflib over the cloned original), then PROVES it in the sandbox, retrying up to `setting.code_author_max_attempts` before recording an honest gap. The model supplies intent; the tooling supplies correct patch mechanics, so a hand-computed hunk header can never break `git apply` (the branch#1667 failure) AND a large file is never re-emitted whole — bounding the model's output to the change so full-file emission cannot blow the output-token cap (the branch#1837 max_tokens truncation). A search block that does not match the file verbatim is caught and fed to the bounded retry, never applied silently. Bounded: only the lab's OWN allowlisted repo (`yoheinakajima/activegraph-lab` — never the packs repo or core, ADR-005); only defects with concrete evidence; gated like any branch (the operator activates); only while the code lane is live; and capped at `setting.max_self_repair_branches` concurrent self-repair branches. The resulting `submit_pr` stays doubly-gated and MCP-excluded (a proven fix opens only a PENDING decision; a fix that fails its sandbox proof — supplied OR authored — opens none) — every PR is the operator's tap. Proof integrity (ADR-039, hardened by ADR-040): when an authored change adds or modifies a test — a pytest file OR a fixture `.yaml` — the proof is RED-THEN-GREEN: the authored test MUST FAIL against the unfixed baseline clone (red) and PASS after the fix (green), with the full suite green after. `proven = baseline-red AND afterfix-green AND suite-green`. A test green on the baseline proves nothing (it is unexecuted, vacuous, or does not exercise the defect) — UNPROVEN, opens no `submit_pr`. Both test forms are held to the same standard; a self-repair that cannot make its regression test fail-then-pass earns no PR. The authoring step receives the FULL target file (via `clone_and_read`), never a truncated brief. A `pr_failed` records a `failure_class` distinguishing a credential wall (Bad credentials / 401 / 403) from other git/API errors so the next attempt is self-classifying.

## Standing behaviors (ADR-044)

- The lab is reactive — the log is the agent, no orchestrator, no scheduler. The ONE exception is the daily heartbeat (`lab_pack/heartbeat.py`, ADR-044): a wall-clock window check (not a loop) that fires at most once per cadence window, idempotent off the log (the window key rides in the `heartbeat.fired`/`heartbeat.skipped` event payload), and brings in ONE step of fresh input before the reactive planner reacts. It is a standing input source, not a control plane: nothing it emits triggers the next tick.
- THE GATE IS UNCHANGED under the heartbeat: everything the heartbeat causes still lands as PROPOSALS in the operator inbox. The heartbeat NEVER approves, promotes, or submits_pr. It cannot self-approve anything.
- Bounded and killable: the heartbeat skips when `llm_cost_today` is at/above `heartbeat_budget_ceiling_usd` (recording `heartbeat.skipped:budget`), so it can never approach the daily cap; and it is killable with no deploy via the `heartbeat_cadence` seam ("off") or the global pause. Adding another standing/wall-clock behavior requires an ADR exactly as this one did.
...
- `pytest >= 7.0` — the self-repair sandbox runs an authored pytest regression test directly with `python -m pytest` to prove it red-then-green (ADR-039/040); the proof harness does not collect pytest files, so pytest must be present in the runtime the sandbox clones into. (Authored fixture `.yaml` tests are exercised by the suite runner itself.)
- No numpy.
