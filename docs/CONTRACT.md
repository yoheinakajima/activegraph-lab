...
- `pytest >= 7.0` — the self-repair sandbox runs an authored regression test directly with `python -m pytest` to prove it executes-and-passes (ADR-039); the proof harness does not collect pytest files, so pytest must be present in the runtime the sandbox clones into.
- No numpy.
